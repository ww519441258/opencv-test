...
function run()
{
    ...
    $cascadeClassifier->load('xml/haarcascade_frontalface_alt2.xml');//加载人脸识别分类器
//    $cascadeClassifier->load('models/lbpcascades/lbpcascade_frontalface.xml');//加载人脸识别分类器
    ...
    while (!$isStop) {
        ...
        $gray = cvtColor($frame, COLOR_BGR2GRAY);//转为灰度图，加快检测

        equalizeHist($gray, $gray);  //直方图均衡化
        $faces = null;
        $cascadeClassifier->detectMultiScale($gray, $faces, 1.1, 3, CV_HAAR_SCALE_IMAGE, new Size(50, 50));
        $face = null;
        $textLb = null;
...
//                    $isStop = true;
//                }
                $textLb = new Point($faces[$i]->x, $faces[$i]->y - 10); //维点模板类
